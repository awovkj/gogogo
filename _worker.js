...
const DASHBOARD_TITLE = "烈火控制台"; //修改你的管理后台标题
...
const handle = (ws, session) => {
  const { proxyIP: pip, sq, enSq, gp, uid } = session;
...
export class ProxySessionDO extends DurableObject {
  async fetch(request) {
    if ((request.headers.get('Upgrade') || '').toLowerCase() !== 'websocket') {
      return new Response('Expected WebSocket', { status: 426 });
    }
    const uid = request.headers.get('x-proxy-uuid') || '';
    if (!uid) return new Response('Missing proxy session config', { status: 400 });
    const url = new URL(request.url);
    const { proxyIP, sq, enSq, gp } = parsePC(url.pathname);
    const { 0: client, 1: server } = new WebSocketPair();
    server.accept();
    server.binaryType = 'arraybuffer';
    handle(server, { proxyIP, sq, enSq, gp, uid });
...
export default {
  async fetch(r, env, ctx) { 
    try {
      ...
      // 🟢 代理入口 - Durable Object 会话承载
      const sessionKey = `${_UUID}:${url.pathname}:${clientIP || 'unknown'}:${crypto.randomUUID()}`;
      const stub = env.PROXY_SESSIONS.get(env.PROXY_SESSIONS.idFromName(sessionKey));
      const headers = new Headers(r.headers);
      headers.set('x-proxy-uuid', _UUID);
      return await stub.fetch(new Request(r, { headers }));
...
<body>
    <div class="login-card">
        <h1>${_s(pageTitle)}</h1>
        <p class="subtitle">请输入管理密码后继续。</p>
        <input type="password" id="pwd" placeholder="请输入密码" autofocus autocomplete="new-password" onkeydown="if(event.key==='Enter')verify()">
        <div class="btn-group">
            <button class="btn-primary" onclick="verify()">登录后台</button>
            <button onclick="window.open('${_j(siteUrl)}', '_blank')">天诚网站</button>
        </div>
        <div class="links">
            <a href="javascript:void(0)" onclick="gh()" class="link-btn">项目直达</a>
            <a href="${_s(tgGroup)}" target="_blank" rel="noopener" class="link-btn">交流群</a>
        </div>
    </div>
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after {
        ...
        .log-time { color: #2563eb !important; }
        .log-ip { color: #0f172a !important; }
        .log-loc { color: #475569 !important; }
        .log-entry { border: 1px solid var(--border) !important; }
        .status-dot,
        .status-indicator {
            box-shadow: none !important;
        }
        ...
        .source-tag.sys { color: var(--warning) !important; }
        .source-tag.man { color: var(--success) !important; }
